...
(define-private (validate-charity-id (charity-id uint))
    (and (> charity-id u0) (<= charity-id (var-get charity-counter)))
)

(define-private (validate-donation-id (donation-id uint))
    (and (> donation-id u0) (<= donation-id (var-get donation-counter)))
)
...
(define-private (validate-principal-address (address principal))
    (and (not (is-eq address (as-contract tx-sender)))
...
(define-public (register-charity (name (string-ascii 100)) (description (string-ascii 500)) 
    ...
    (let
        ((new-charity-id (+ (var-get charity-counter) u1))
         ...
         (validated-wallet-address wallet-address)
         (validated-verification-docs verification-docs)
         (validated-impact-category impact-category))
        
        (asserts! (< (var-get charity-counter) MAX-CHARITIES) ERR-INVALID-AMOUNT)
        (asserts! (validate-string-not-empty validated-name) ERR-INVALID-INPUT)
        (asserts! (validate-string-not-empty validated-description) ERR-INVALID-INPUT)
        (asserts! (validate-string-not-empty validated-verification-docs) ERR-INVALID-INPUT)
        (asserts! (validate-string-not-empty validated-impact-category) ERR-INVALID-INPUT)
        (asserts! (validate-principal-address validated-wallet-address) ERR-INVALID-INPUT)
        
        (map-set charities { charity-id: new-charity-id }
            {
                name: validated-name,
                description: validated-description,
                wallet-address: validated-wallet-address,
                registration-block: block-height,
                is-active: true,
                total-received: u0,
                beneficiaries-helped: u0,
                verification-documents: validated-verification-docs,
                impact-category: validated-impact-category
            })
...
(define-public (make-donation (charity-id uint) (amount uint) (purpose (string-ascii 200)) (allocation-percentage uint))
    (let
        ((new-donation-id (+ (var-get donation-counter) u1))
         ...
         (validated-charity-id charity-id)
        ...
        (asserts! (validate-charity-id validated-charity-id) ERR-CHARITY-NOT-FOUND)
        (asserts! (validate-string-not-empty validated-purpose) ERR-INVALID-INPUT)
        
        (match (map-get? charities { charity-id: validated-charity-id })
            charity-data
            (begin
                ...
                (map-set donations { donation-id: new-donation-id }
                    {
                        donor: tx-sender,
                        charity-id: validated-charity-id,
                        amount: amount,
                ...
                (map-set charities { charity-id: validated-charity-id }
...
(define-public (report-donation-impact (donation-id uint) (beneficiaries-reached uint) 
    ...
    (let
        ((validated-donation-id donation-id)
         (validated-impact-description impact-description)
         (validated-evidence-hash evidence-hash)
         (validated-measurable-outcomes measurable-outcomes))
        
        (asserts! (validate-donation-id validated-donation-id) ERR-DONATION-NOT-FOUND)
        (asserts! (validate-string-not-empty validated-impact-description) ERR-INVALID-INPUT)
        (asserts! (validate-string-not-empty validated-evidence-hash) ERR-INVALID-INPUT)
        (asserts! (validate-string-not-empty validated-measurable-outcomes) ERR-INVALID-INPUT)
        (asserts! (<= impact-score MAX-IMPACT-SCORE) ERR-INVALID-PERCENTAGE)
        (asserts! (> beneficiaries-reached u0) ERR-INVALID-BENEFICIARIES)
        
        (match (map-get? donations { donation-id: validated-donation-id })
            donation-data
            (let
                ...
                ;; Record impact data
                (map-set donation-impacts { donation-id: validated-donation-id }
                    {
                        beneficiaries-reached: beneficiaries-reached,
                        impact-description: validated-impact-description,
                        evidence-hash: validated-evidence-hash,
                        verification-date: block-height,
                        impact-score: impact-score,
                        measurable-outcomes: validated-measurable-outcomes
                    })
                
                ;; Update donation status
                (map-set donations { donation-id: validated-donation-id }
...
(define-public (add-project-milestone (charity-id uint) (milestone-id uint) (description (string-ascii 200))
    ...
    (let
        ...
        (asserts! (validate-charity-id validated-charity-id) ERR-CHARITY-NOT-FOUND)
        (asserts! (validate-milestone-id validated-milestone-id) ERR-INVALID-MILESTONE)
        (asserts! (validate-string-not-empty validated-description) ERR-INVALID-INPUT)
        ...
        (match (map-get? charities { charity-id: validated-charity-id })
            ...
            (begin
                (asserts! (is-eq tx-sender (get wallet-address charity-data)) ERR-NOT-AUTHORIZED)
                (asserts! (is-none (map-get? project-milestones { charity-id: validated-charity-id, milestone-id: validated-milestone-id })) ERR-INVALID-MILESTONE)
                
                (map-set project-milestones { charity-id: validated-charity-id, milestone-id: validated-milestone-id }
                    {
                        description: validated-description,
...
(define-public (complete-milestone (charity-id uint) (milestone-id uint) (verification-evidence (string-ascii 200)))
    (let
        ((validated-charity-id charity-id)
         (validated-milestone-id milestone-id)
         (validated-verification-evidence verification-evidence))
        
        (asserts! (validate-charity-id validated-charity-id) ERR-CHARITY-NOT-FOUND)
        (asserts! (validate-milestone-id validated-milestone-id) ERR-INVALID-MILESTONE)
        (asserts! (validate-string-not-empty validated-verification-evidence) ERR-INVALID-INPUT)
        
        (match (map-get? charities { charity-id: validated-charity-id })
            charity-data
            (match (map-get? project-milestones { charity-id: validated-charity-id, milestone-id: validated-milestone-id })
                milestone-data
                (begin
                    (asserts! (is-eq tx-sender (get wallet-address charity-data)) ERR-NOT-AUTHORIZED)
                    ...
                    (map-set project-milestones { charity-id: validated-charity-id, milestone-id: validated-milestone-id }
                        (merge milestone-data {
                            ...
                            is-completed: true,
                            verification-evidence: validated-verification-evidence
                        }))
                    (ok true))
...
(define-public (update-charity-verification (charity-id uint) (verification-level uint) 
    ...
    (let
        ...
        (map-set charity-verifications { charity-id: validated-charity-id }
            {
